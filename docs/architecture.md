...
│    chat-skills.ts  builds Claude account upload packages, tracks state     │
│    zip.ts          a minimal ZIP writer, so packaging adds no dependency   │
...
Better Claude Setup configures two things that Anthropic keeps deliberately separate:

| | Claude Code | Claude account |
| --- | --- | --- |
| Storage | files under `~/.claude` | uploaded to the account |
| Written by | this app, directly | the user, guided by this app |
| Verified by | reading the files back | the user's confirmation only |
| Reaches | CLI, IDEs, desktop Code tab | web chat, desktop Chat, Cowork |

They do not sync — Anthropic's documentation says so explicitly — so the app models them as
two surfaces with two states rather than one "installed" flag.

The account side stops at a hard boundary. There is no documented consumer API for adding a
skill to a personal Claude account, so the app builds the upload packages and guides the
step. It never reads a Claude session, cookie, token or local Claude database to close the
gap, and its state for that surface is called "confirmed by you" in the interface because
that is exactly what it is.

`chat-skills.ts` fingerprints package contents with a hash, which is what lets the manager
say "update available" honestly: it compares what the app would now produce against what
the user last confirmed uploading, never against the account.

## One canonical skill source

`content.ts` holds one definition per skill and exports per surface:

```
CanonicalSkill ──> toClaudeCodeSkill()  ──> ~/.claude/skills/<command>/SKILL.md
               ──> toChatSkill()        ──> <command>.zip  (account upload)
               ──> toLegacyAliasSkill() ──> the v1.1 command name, optional
```

The frontmatter differs by surface — Anthropic documents a 200 character description limit
for account skills against Claude Code's far looser listing budget — but the body is written
once. Two hand-maintained copies would drift, and `/research` would quietly come to mean
different things in different places.

## The native-control safe area

The window draws its own title strip while the operating system draws its controls on top.
Keeping content out from under them is not a margin to be tuned by eye:

- **Windows** publishes the overlay geometry to CSS as `env(titlebar-area-x)` and
  `env(titlebar-area-width)`. Those already account for display scaling and for the controls
  changing size, so they are the only correct source.
- **macOS** does not publish them for an inset title bar, but its traffic lights sit at a
  known inset and disappear in fullscreen, which the main process reports over IPC.

`scripts/titlebar-probe.cjs` measures the content edge against the safe edge at several
window widths, and is run at 100%, 125% and 150% display scale.
