...
export async function buildDiagnosticReport(env: Env, appVersion: string): Promise<string> {
  ...
  lines.push('')
  lines.push('## Claude')
  lines.push(`- Claude Code: ${scan.claudeCode.state}`)
  ...
  lines.push(`- Claude desktop app: ${scan.claudeDesktop.state}`)
  lines.push(`- Claude desktop version: ${scan.claudeDesktop.version ?? 'unknown'}`)
  lines.push(`- Claude desktop found via: ${scan.claudeDesktop.foundVia ?? 'not found'}`)
  lines.push('- Detection routes tried:')
  for (const p of scan.claudeDesktop.probes) {
    lines.push(`  - ${p.method}: ${p.found ? 'yes' : 'no'} (${p.note})`)
  }
  lines.push('')
